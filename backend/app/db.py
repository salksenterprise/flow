...
DEFAULT_DB = Path(__file__).resolve().parents[1] / "workflow.db"


def db_path() -> Path:
    return Path(os.environ.get("WORKFLOW_DB_PATH", DEFAULT_DB))
...
CREATE TABLE IF NOT EXISTS workflow_version (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  definition_id INTEGER NOT NULL REFERENCES workflow_definition(id),
...
CREATE TABLE IF NOT EXISTS transition_definition (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  workflow_version_id INTEGER NOT NULL REFERENCES workflow_version(id),
  from_step_id INTEGER NOT NULL REFERENCES step_definition(id),
  to_step_id INTEGER NOT NULL REFERENCES step_definition(id),
  condition_json TEXT,
...
CREATE TABLE IF NOT EXISTS step_instance (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  workflow_instance_id INTEGER NOT NULL REFERENCES workflow_instance(id),
  step_definition_id INTEGER NOT NULL REFERENCES step_definition(id),
...
CREATE INDEX IF NOT EXISTS idx_event_workflow ON workflow_event(workflow_instance_id, id);
"""
